        ...
        <CardContent className="p-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {paginated.length === 0 ? (
                  <tr>
                    <td colSpan={10} className="text-center text-gray-400 py-8">No appointments found.</td>
                  </tr>
                ) : (
                  paginated.map((a, idx) => (
                      ...
                      <td className="py-4 px-2">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem onClick={() => handleEdit(a, idx)} className="flex items-center gap-2">
                              <Pencil className="h-4 w-4" /> Edit
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => handleDelete(a)} className="text-red-600 flex items-center gap-2">
          ...
          <div className="w-full flex justify-end mt-4">
            <div className="flex items-center gap-2 text-sm text-gray-700 ">
              ...
              <span className="font-medium">{filtered.length === 0 ? 0 : page * rowsPerPage + 1}-{Math.min((page + 1) * rowsPerPage, filtered.length)}</span>
      ...
      <Dialog open={editModalOpen} onOpenChange={setEditModalOpen}>
        <DialogContent className="max-w-lg p-0  rounded-2xl">
          <form onSubmit={handleEditSave} className="p-8 flex flex-col gap-4">
            <DialogHeader>
              <DialogTitle className="text-2xl font-bold text-center text-[#012765]">Edit Notes</DialogTitle>
            </DialogHeader>
            ...
            <div className="flex justify-between mt-6">
              ...
              <Button type="submit" className="bg-[#FF7119] text-white px-8 py-2 font-bold rounded-md shadow">Save</Button>
            </div>
          </form>
        </DialogContent>
      </Dialog>
